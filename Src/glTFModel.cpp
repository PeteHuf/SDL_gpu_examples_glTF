...
namespace vkglTF
{
	...
	void Texture::fromglTfImage(tinygltf::Image &gltfimage, std::string path, TextureSampler textureSampler, vks::VulkanDevice *device)
	{
		...
		//else
		{
			...
			SDL_memcpy(textureTransferPtr, gltfimage.image.data(), width * height * 4);
			SDL_UnmapGPUTransferBuffer(this->device->logicalDevice, textureTransferBuffer);
			...
			view = SDL_CreateGPUTexture(this->device->logicalDevice, &textureCreateInfo);
		...
		SDL_GPUSamplerCreateInfo samplerCreateInfo{
			...
			.mag_filter = textureSampler.magFilter,
			.mipmap_mode = SDL_GPU_SAMPLERMIPMAPMODE_LINEAR,
			.address_mode_u = textureSampler.addressModeU,
			.address_mode_v = textureSampler.addressModeV,
			.address_mode_w = textureSampler.addressModeW,
		};

		sampler = SDL_CreateGPUSampler(this->device->logicalDevice, &samplerCreateInfo);
